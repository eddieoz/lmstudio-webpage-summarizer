  ...
  try {
    ...
    let systemCommand =
      mode === "summarize"
        ...
        : "Task: Explain following the article. Format: Markdown. Requirements: explain all the main ideas, key points and tech terminology; use simple terms that a beginner can easily understand; explanation should be free from ambiguity. You will always follow these structured instructions: Break down tasks by using Chain of Thought (CoT) reasoning, articulating each logical step and verifying them for coherence. Apply Step-by-Step Rationalization (STaR) to justify decisions, balancing depth with efficiency. Integrate A Search* principles to optimize your approach, evaluating the efficiency of potential paths and selecting the most direct strategy. Use Tree of Thoughts (ToT) to explore multiple solutions in parallel, evaluating and converging on the most promising one. Simulate Adaptive Learning by reflecting on decisions as if learning from outcomes, prioritizing strategies that yield the best results. Continuously monitor your process, assessing progress to ensure alignment with the overall goal and refining your approach as needed. Your ultimate goal is to deliver the most logical, effective, and comprehensive solution possible by fully integrating these advanced reasoning techniques. Explain in " + lang + " language. Article: ";
